...
export const getRiskScore = async (lat, lng) =>
  callApi(`/api/risk?lat=${lat}&lng=${lng}`, {}, { score: Math.floor(Math.random() * 40) + 60 });

export const getRiskDetailed = async (lat, lng) =>
  callApi(`/api/risk/detailed?lat=${lat}&lng=${lng}`, {}, {
...
function mockEmergencyList(city, lat, lng) {
  ...
  return [
    { name: hospitals[h % hospitals.length], type: 'hospital', distance: km(h), eta: et(h + 1) },
    { name: police[(h >> 3) % police.length], type: 'police', distance: km(h + 7), eta: et(h + 5) },
  ];
}
